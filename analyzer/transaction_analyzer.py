...
from domain_models import TransactionDomainModel, EthereumTransactionType, TransactionListDomainModel
...
class EthereumTransactionAnalyzer(BaseTransactionAnalyzer):
    ...
    def _process_transaction(self, tx: Dict[str, Any], transaction_type: str) -> TransactionDomainModel:
        """
        ...
        return TransactionDomainModel(
            ...
            asset_symbol="ETH" if transaction_type == "ETH Transfer" else tx.get('tokenSymbol', ''),
            ...
            gas_fee_eth=calculate_gas_fee(tx.get('gasUsed', '0'), tx.get('gasPrice', '0')),
            is_error=tx.get('isError', '0') == '1'
        )

    def analyze(self, raw_transaction_data: Dict[str, List[Dict[str, Any]]]) -> TransactionListDomainModel:
        ...
        for tx_type, transactions in raw_transaction_data.items():
            if tx_type in self.TRANSACTION_TYPES_DISPLAY_NAMES:
                ...
                for tx in transactions:
                    try:
                        processed_tx = self._process_transaction(tx, self.TRANSACTION_TYPES_DISPLAY_NAMES[tx_type])
