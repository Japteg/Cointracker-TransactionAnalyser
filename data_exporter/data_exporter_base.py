...
class DataExporterBase(ABC):
    ...
    def _validate_data(self, transactions: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
                    ...
                    if column == 'date_time':
                        ...
                        if value:
                            try:
                                ...
                                logger.warning(f"Invalid date format in transaction {i}: {value}")
                                value = ''
        ...
        logger.info(f"Validated {len(validated_transactions)} transactions out of {len(transactions)}")
    ...
    def _generate_filename(self, address: str) -> str:
        ...
        short_address = address[:6] + '...' + address[-4:] if len(address) > 10 else address
